...
RUN R -e "update.packages(ask=F);install.packages("BiocManager"); BiocManager::install(c('dplyr','plotly','networkD3','data.table','stringr','DT','MASS','pracma','preprocessCore','limma','qvalue','colourpicker','shinydashboard','shinyBS','heatmaply','GGally','rmarkdown','shinycssloaders','cowplot','pander'),ask=F)"
